...
for %%E in (%encoder_list%) do (
    set "encoder=%%E"
     ...
     if "!encoder!"=="hevc_nvenc" (
        set "vopts=-c:v hevc_nvenc -preset p7 -rc constqp -qp 27 -r 25 !vf!"
    ) else if "!encoder!"=="hevc_amf" (
        set "vopts=-c:v hevc_amf -quality quality -qp_p 27 -qp_i 27 -qp_b 27 -pix_fmt yuv420p -r 25 !vf!"
    ) else if "!encoder!"=="hevc_qsv" (
        set "vopts=-c:v hevc_qsv -global_quality 27 -look_ahead 1 -pix_fmt nv12 -r 25 !vf!"
    )
